...
def main():
    ...
    Prefix = ""
    OutPath = ""
    opts, args = getopt.getopt(sys.argv[1:], "hd:p:o:", ["help","Dir=", "Prefix=","OutPath="])
    if len(opts) == 0:
    ...
    for opts, arg in opts:
        if opts == "-h" or opts == "--help":
            ...
            exit()
        elif opts == "-d" or opts == "--Dir":
            dir= str(arg)
        elif opts == "-p" or opts == "--Prefix":
            Prefix = str(arg)
        elif opts == "-o" or opts == "--OutPath":
            OutPath = arg
    return dir,Prefix,OutPath
...
def drow_bar(Clust_pos_list,colors,as_plot1,as_plot2):
    clusts = Clust_pos_list[:,0].astype(int)
    phi_x = Clust_pos_list[:,1].astype(float)
    fq_max = np.max(fq(phi_x))
    last = -1
    last_index = []
    for c_id in np.unique(clusts):
        indx = np.argwhere(c_id==clusts).reshape(-1)
        x = phi_x[indx]
        clust_max = np.max(fq(x))
        sigma = np.std(x)*0.8
        mu = np.mean(x)
        if last-mu < 0.05 and last-mu >0:
            index_merge = np.r_[last_index,indx]
            x = phi_x[index_merge]
            clust_max1 = np.max(fq(x))
            sigma1 = np.std(x)
            mu1 = np.mean(x)
            x = np.linspace(-0.01, 1.01, 102)
            y = norm(mu1, sigma1).pdf(x)
            y = y * (clust_max1 / np.max(y))/fq_max
            as_plot1.plot(x, y, color="gray", linestyle='-',linewidth=3)
        last_index = indx
        last = mu
        x = np.linspace(-0.01, 1.01, 102)
        y = norm(mu, sigma).pdf(x)
        y = y * (clust_max / np.max(y))/fq_max
        as_plot1.plot(x, y, color=colors[c_id], linestyle='-.')
    last = -1
    last_index = []
    phi_x = Clust_pos_list[:, 2].astype(float)
    for c_id in np.unique(clusts):
        indx = np.argwhere(c_id == clusts).reshape(-1)
        x = phi_x[indx]
        clust_max = np.max(fq(x))
        ...
        if last-mu < 0.05 and last-mu >0:
            ...
            y = norm(mu1, sigma1).pdf(x)
            y = y * (clust_max1 / np.max(y))/fq_max
            as_plot2.plot(x, y, color="gray", linestyle='-',linewidth=3)
        last_index = indx
        last = mu
        x = np.linspace(-0.01, 1.01, 102)
        y = norm(mu, sigma).pdf(x)
...
dir,Prefix,OutPath = main()
clust_pos = str(dir)+str(Prefix)+".Clust_pos.txt"
summary = str(dir)+str(Prefix)+".summary_table.txt"
pos_table = read_profile(clust_pos)
summary_table = read_profile(summary)
...
as_raw.set_xlabel("CCF of EGAF00000057355",fontsize=18)
as_raw.set_ylabel("CCF of EGAF00000098595",fontsize=18)
...
points = pos_table[:,4:7]
colors = sns.color_palette(n_colors=30)
drow_bar(points,colors,as_plot1,as_plot2)

x2 = points[:, 1].astype(float)
y2 = points[:, 2].astype(float)
types = points[:,0]
typ = np.unique(types)

clas = np.zeros([len(types)])
for i in range(len(typ)):
    index_s =(summary_table[:, 0] == typ[i]).reshape(-1)
    index = np.argwhere(types==typ[i]).reshape(-1)
    x2_1 = x2[index]
    y2_1 = y2[index]
    as_raw.scatter(x2_1,y2_1,c=colors[i],alpha=0.8,s=12)
    if len(index) > 0:
        cen_x = float(summary_table[index_s,2])
        cen_y = float(summary_table[index_s,3])
        as_raw.scatter(cen_x, cen_y, c="black", alpha=0.8, s=20)

plt.savefig("./output_real/merge6.png")
plt.close()